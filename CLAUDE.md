...
- ElevenLabs: chave `sk_` (51 chars) na env var `ELEVENLABS_API_KEY` do environment; o setup grava em `.env` na raiz do video-use. Escopos necessários: TTS, STT Scribe, sound-generation, voices_read. Plano e cota (conferidos em 2026-08-18): **FREE, 10.000 chars/mês** (261 usados), 3 voice slots, sem instant voice cloning. Escopos TTS, STT e sound-generation confirmados ativos na chave. Cota curta: usar Scribe e sound-generation com parcimônia até upgrade. Voz da marca para narração: **PENDENTE (voice_id, modelo e settings)**.
